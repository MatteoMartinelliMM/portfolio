...
function ProjectCard({project}) {
    ...
    return project.url ? (
        ...
        <CardContent project={project} isExpanded={isExpanded} setIsExpanded={setIsExpanded}/>
    );
}

function CardContent({project, isExpanded, setIsExpanded}) {
    return <div
        className="text-start card card-compact  masonry-item !p-0 bg-base-100 shadow-xl break-inside-avoid mx-4  overflow-hidden">
        <figure>
            <img src={project.imageUrl} className={`${project.url ? 'clickable-card' : ''}`}/>
        </figure>
        <div className="card-body !p-8 whitespace-pre-line">
            <h2 className="card-title">{project.title}
                <div
                    className={`badge ${isPublic(project) ? 'badge-success' : 'badge-error'}`}>
                    <svg  viewBox="0 0 15 15" fill="none"
                         xmlns="http://www.w3.org/2000/svg"  className="inline-block h-4 w-4">
                        <path
